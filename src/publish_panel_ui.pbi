...
;╚═════════════════════════════════════════════════════════════════════════════════════════════════
DeclareModule PublishPanelUI
  
  Declare.i CreatePublishPanel(hWindow.i) ; Creates the publish panel
  
  Declare.s PublishPanelUIError() ; Returns last error generated by the library

EndDeclareModule

Module PublishPanelUI
  EnableExplicit
  ...
  Procedure CreateTitleBar()
    ...
    If IsGadget(hTitleBar)
      ...
      hFont = LoadFont(#PB_Any, "", #PANEL_TITLE_BAR_TEXT_HEIGHT)
        ...
        hTitleLabel = TextGadget(#PB_Any, #TITLE_LABEL_X, #TITLE_LABEL_Y, 50, #PANEL_TITLE_BAR_IMAGE_HEIGHT, "Publish Database")
  ...
  Procedure.s PublishPanelUIError()
    Shared lastError$
  ...
  ;
  Procedure.i CreatePublishPanel(hWindow.i) 
    ...
    If IsWindow(hWindow)
      ...
      lastError$ = "Failed to create publish panel, invalid ID returned"
      ProcedureReturn 0         
    EndIf
    
    lastError$ = "Invalid window ID passed to CreatePublishPanel function"
    ProcedureReturn 0    
  EndProcedure
  
  DataSection
    LogoImage:
    IncludeBinary #PB_Compiler_FilePath + "res/panels/publish@48px.png"
  EndDataSection  
EndModule
